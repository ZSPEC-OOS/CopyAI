...
/**
 * Scripts has no login of its own — it rides on the CopyAI session. This
...
export function ScriptsGate() {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect -- one-time sync with the auth session before mounting ScriptsApp, not derivable during render
    setNamespaceReady(true);
  }, [checked, uid, router]);

  if (!checked || !uid || !namespaceReady) {
    return (
      <div className="scripts-root scripts-loading-screen">
        <span className="scripts-loading-screen__logo" aria-hidden="true">
          ⟡
        </span>
